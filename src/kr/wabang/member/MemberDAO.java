...
public class MemberDAO extends DBConnection implements MemberInterface  {
	...
	public MemberVO myInfoSelect(String userid) {
		...
		try{
			dbCon();
			String sql = "select m_name, m_tel, m_email, m_zipcode, m_addr, m_addrDetail, m_addrReference "
			...
			rs = pstmt.executeQuery();
			if(rs.next()){
				vo.setM_name(rs.getString(1));
				vo.setM_tel(rs.getString(2));
				vo.setM_email(rs.getString(3));
				vo.setM_zipCode(rs.getString(4));
				vo.setM_addr(rs.getString(5));
				vo.setM_addrDetail(rs.getString(6));
				vo.setM_addrReference(rs.getString(7));
			}
	...
	public int myInfoUpdate(MemberVO vo) {
		...
		try{
			dbCon();
			String sql = "update member set m_name=?, m_zipCode=?, m_addr=?, m_addrDetail=?, m_addrReference=?, m_tel=?, m_email=? "
					+ "where m_id=? and m_pwd=?";
			pstmt = con.prepareStatement(sql);
			
			pstmt.setString(1, vo.getM_name());
			pstmt.setString(2, vo.getM_zipCode());
			pstmt.setString(3, vo.getM_addr());
			pstmt.setString(4, vo.getM_addrDetail());
			pstmt.setString(5, vo.getM_addrReference());
			pstmt.setString(6, vo.getM_tel());
			pstmt.setString(7, vo.getM_email());
